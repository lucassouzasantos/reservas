/*
  ...
  4. Dados Iniciais
    - Inserir salas existentes com arrays corretos
*/
...
-- Inserir salas existentes apenas se não existirem (usando sintaxe correta para arrays)
DO $$
BEGIN
  -- Inserir sala 1
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 1) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (1, 'Sala del consejo', 10, 'Oficina Central', ARRAY[]::text[]);
  END IF;
  
  -- Inserir sala 2
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 2) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (2, 'Auditorio', 100, '2 Piso ECOP', ARRAY[]::text[]);
  END IF;
  
  -- Inserir sala 3
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 3) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (3, 'Sala Insumos', 20, 'Insumos Central', ARRAY[]::text[]);
  END IF;
  
  -- Inserir sala 4
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 4) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (4, 'Amambay', 10, 'Insumos Amambay', ARRAY[]::text[]);
  END IF;
  
  -- Inserir sala 5
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 5) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (5, 'San Juan nepomuceno', 12, 'Oficina San Juan Nepomuceno', ARRAY[]::text[]);
  END IF;
  
  -- Inserir sala 6
  IF NOT EXISTS (SELECT 1 FROM salas WHERE id = 6) THEN
    INSERT INTO salas (id, nome, capacidade, ubicacion, equipamento) 
    VALUES (6, 'Sala Piso 3', 8, 'Piso 3', ARRAY[]::text[]);
  END IF;
END $$;
